...
class bolBreak: public expert{
   ...
   public:
      bolBreak(int period_,int longPR_,double ratio_,double sl_,double tp_,bool LP,bool SWL,
         string sym,ENUM_TIMEFRAMES per,double C,bool everyTick,bool t,bool stackM,double W,double F,ulong magic);
};

bolBreak::bolBreak(int period_,int longPR_,double ratio_,double sl_,double tp_,bool LP,bool SWL,
         string sym,ENUM_TIMEFRAMES per,double C,bool everyTick,bool t,bool stackM,double W,double F,ulong magic){
   period=period_;longPR=longPR_;ratio=ratio_;sl=sl_;tp= tp_;longP=LP;stopWithLong=SWL;
   init(sym,per,C,everyTick,t,stackM,W,F,magic);  
}



void bolBreak::initIndi(string s){
      band.Create(s,m_period,period,0,ratio,PRICE_CLOSE);
      m_indicators.Add(GetPointer(band));
      sd.Create(s,m_period,period,0,MODE_SMA,PRICE_CLOSE);
      m_indicators.Add(GetPointer(sd));
      ma.Create(s,m_period,longPR*period,0,MODE_SMA,PRICE_CLOSE);
